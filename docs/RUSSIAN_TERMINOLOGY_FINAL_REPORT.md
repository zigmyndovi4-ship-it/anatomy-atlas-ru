...
## Linguistic cleanup pass

Проведён отдельный глобальный проход по явным языковым дефектам без пересмотра
допустимых вариантов анатомической терминологии. Проверялись род, согласование,
порядок слов, остатки английского и машинной транслитерации. Изменены 371
термина с высокой уверенностью; `conceptId`, `en`, `atlas.json`, геометрия,
лицензии и attribution не изменялись.

Текущий review-артефакт: `scripts/translation/linguistic-cleanup-review.tsv`.
После применения HIGH-исправлений в нём осталось 39 записей `REVIEW`; это
преимущественно стоматологические формулировки, FMA aggregate/cluster names,
редкие нейроанатомические структуры и несколько форм, для которых нужен
специализированный русский источник. Они намеренно не исправлялись предположительно.

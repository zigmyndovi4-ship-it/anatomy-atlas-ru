...
const reviewFile = 'scripts/translation/final-review-39.tsv';
...
for (const row of rows) {
  ...
  if (data[row.conceptId].en !== row.en) throw new Error(`English mismatch: ${row.conceptId}`);
  if (data[row.conceptId].ru !== row.current_ru) throw new Error(`Current ru mismatch: ${row.conceptId}`);
